...
.home-container {
  ...
  .content {
    ...
    .text {
      ...
      /* 白字 14px 需要 4.5:1：透明底上渐变最亮端（t=0）白字只有 3.1193:1，远低于正文门槛，
         故用 --scrim-30 深色底把白字抬高。抬高后全带最低 4.5103:1（同样在 t=0），
         即整条渐变上任意位置都达标，不依赖按钮实际落在哪一段；悬停再深一档
         （--scrim-40，t=0 处 5.1440:1）。聚焦环在彩色渐变上不能用主色（与背景同色系不可辨），
         改用纯白。 */
      .ghost-btn {
