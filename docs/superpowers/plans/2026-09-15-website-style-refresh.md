...
> **发送按钮保留渐变，不要压成纯色。** 该按钮内部**只有图标、没有文字**（`template:185-189` 为 `el-button > el-icon > Promotion`），因此它是**非文字的功能性元素**，门槛是 **3:1 而非 4.5:1**，而 `--color-accent` → `--color-accent-text` 的渐变最浅端就有 **3.2733:1**，全段达标（中点 3.9692:1、深端 4.8373:1）。把渐变拉平成纯色属于过度修正，且会让"发送按钮为橙色渐变"这一验收项失效。**只有"控件内含 <24px 文字且文字为浅色"时才需要 4.5:1**——判断依据是按钮里有没有文字节点，不是按钮本身。
...
**补充：本页有 4 个红色字面量不在上面任何一组里**（`.error-message` 区域，各 1 处，合计正是 94 + 4 = 98）。原稿的四组只能覆盖 94 项，这是原稿的遗漏，不是文件的问题：

| 原值 | 替换为 | 说明 |
|---|---|---|
| `#fef2f2` | `var(--color-danger-light)` | 错误提示条底色 |
| `#fecaca` | `var(--color-danger)` | 错误提示条描边 |
| `#f87171` | `var(--color-danger)` | 图标/强调（非文字，3:1 门槛，达标） |
| `#991b1b` | `var(--color-text)` | 错误提示的**正文文字**。**不能**用 `--color-danger`：它在 `--color-danger-light` 上实测仅 **3.8510:1**，低于 16px 正文要求的 4.5:1；换 `--color-text` 后为 **10.0821:1** |
...
**实施中发现的硬约束（计划原文在此处不可行，已按实测改写）：这 4 张卡的图标是纯白 PNG。** 逐像素实测（`vue/src/assets/images/{users,like,comments,smile}.png`）：所有 α>0 的像素——**包括抗锯齿像素**——都是 RGB(255,255,255)。因此"浅底 + 深色图标"会让图标**彻底不可见**（白图标对浅底壳 1.12–1.15:1）。正确做法是把**深阶色放在 60×60 的图标壳底色上**（白图标对壳 4.84–5.34:1），卡片表面才用浅阶色，并通过 `.el-card:has(.avatar.<name>)` 命中（已验证：`<template>` 中 `.avatar` 与 `.info` 是兄弟节点，编译后作用域属性落在 `.el-card` 上，`(0,5,0)` 高于 Element 的 `(0,1,0)`）。**即使浏览器不支持 `:has()` 而回退成白卡，四张卡的数字与标题也仍然达标（4.84–5.34:1 / 5.29:1）**，所以该技术不产生不可读的中间态。

**数字的对比度门槛是 3:1 而非 4.5:1**：`.number` 为 `font-size: 24px; font-weight: 700`，同时满足"≥24px 常规"与"≥18.66px 加粗"两个大字号条件。四张卡在各自底色上的实测为 primary **4.56:1**、accent-text **4.31:1**、info **4.65:1**、success **4.41:1**，全部达标。**不要**因为 4.31/4.41 低于 4.5 就把它们改成 `--color-text`——那会丢掉设计里"每张卡一个语义色"的意图，而且是把 4.5:1 门槛错误地套用到大字号上（Task 7 已经因同类过度修正浪费过一轮）。
