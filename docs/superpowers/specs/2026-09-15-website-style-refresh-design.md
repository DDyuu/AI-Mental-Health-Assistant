...
**设计约束的发现过程**：初版主色 `#5B9E8F` 经 WCAG 对比度核算属中间明度色——白字在其上仅 3.12:1，深字也仅 3.81:1（`--color-text` `#2C3A36` 在 `#5B9E8F` 上，实测 3.8130:1），**两者都无法达到 AA 4.5:1**。这是该色调的数学限制，非参数可调。因此采用行业常规的双阶主色方案：柔和感由大面积色块承载，小面积交互元素用深一阶保证可读性。
...
- **作为非文字元素** → **≥ 3:1**，但范围限定为**功能性 / 状态性信息**：焦点环、悬停与选中态、承载语义的图标、图表线条、以及**用于识别表单控件的边框**。中性的结构性分隔（分割线、卡片描边、表头/表格行线）**不属于**该项要求——WCAG 1.4.11 只覆盖"识别组件与状态所必需"的视觉信息，纯装饰与纯结构的边界不在其内。

**两处明文例外（均为已知偏差，不是疏漏）：**

**例外一：`--color-text-placeholder`（占位符与禁用态文字）。** 它在白底上为 2.47:1，**不满足**上文的 4.5:1 文字标准。保留该值是有意的取舍：
...
**例外二：中性结构性分隔线不适用 3:1。** 实测值：`--color-border` `#E6EDEA` 对白 1.19:1、对 `--color-bg` 1.12:1；`--color-border-light` `#F0F4F2` 对 `--color-bg` 1.05:1。把它们压到 3:1 需要接近 `#83918C` 的中灰，会让所有卡片与表格镶上一圈沉重边框，与"柔和治愈"的整体方向直接冲突，而收益是零——1.4.11 并不要求装饰性边界。

**作为例外二的补偿，表单控件的边框单独提升到 3:1**：新增 `--color-border-strong`（`#83918C`，对白 3.28:1、对 `--color-bg` 3.11:1），并把 Element 的 `--el-input-border-color` 指向它、`--el-input-hover-border-color` 指向 `--color-primary`（5.16:1）。理由：输入框的边框是用户识别"这里可以输入"的主要线索，属于 1.4.11 的覆盖范围；而分割线与卡片描边不是。焦点态本就走 `--color-primary`，无需改动。

**仍未覆盖的部分（已知、待评估）**：`el-select`、`el-checkbox`、`el-table` 等的静止态边框仍走软令牌 `--color-border`，未达 3:1。严格按 WCAG 1.4.11 审计会把这些一并标出。本次不做，理由是它们各自的 label / 勾选框 / 表头提供了额外识别线索，而全量加深边框的视觉代价太大。凡日后要做无障碍合规审计，此处与例外一同样需重新评估。

**仍在覆盖范围内、但需要额外覆盖的一类：按钮的悬停 / 按下 / 聚焦状态色。** Element Plus 默认把主按钮的悬停底色设为 `--el-color-primary-light-3`、聚焦环设为 `light-5`。这对浅色主色成立，但本设计的主色 `#40776B` 偏深，算出来的悬停底色 `#79a097` 上白字只有 **2.88:1**、聚焦环 `#a0bbb5` 对白只有 **2.04:1**，两项都不达 3:1 / 4.5:1。因此覆盖为：悬停与按下走向**深阶**（`dark-2`，白字 7.2:1），聚焦环用 `--color-primary`（对白 5.16:1）。这不是可选项——按钮状态是全站每个表单与页面都要用到的功能性与状态性信息。

**聚焦环必须与被聚焦的组件本身形成对比，而不只是与页面背景形成对比。** 这是实施中实测踩到的坑：把 `--el-button-outline-color` 全局设为主色，会让**主按钮的聚焦环与它自己的填充同色**（同一枚 `--color-primary`），实测环对按钮 **1.000:1**、对相邻渐变 1.259:1——键盘用户的聚焦指示器于是彻底消失。正确做法是按底色分别取环色：浅底（白/近白）上的按钮用 `--color-primary`（对白 5.16:1），深色/彩色底上的按钮改用 `--color-text-inverse`（对主色填充 5.16:1、对青绿渐变约 5:1）。凡日后新增"彩色底上的按钮"，都要单独确认其聚焦环。

**已知偏差第三项：幽灵按钮（ghost button）的自身边界不足 3:1。** 实测 `--alpha-60` 白描边合成后对渐变仅 2.43–2.60:1，其 scrim 填充对渐变只有 1.31–1.33:1。本设计接受这一偏差，理由是**该控件由 4.5:1 的文字标签识别**（标签是"开始倾诉/记录心情"这类明确文本），符合 §3.5 对非文字元素范围的收窄。需要如实记录的是：**不能声称"填充边缘承担了识别"**——1.3:1 的色调差在视觉上不构成可辨的填充边界。若日后把 §3.5 的非文字规则重新扩大到边界本身，此按钮不达标。
